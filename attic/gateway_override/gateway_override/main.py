...
async def _demo_check_limits(ip: str) -> None:
    ...
    try:
        ...
        # Always record timestamp (keeps behavior stable)
...
async def _billing_gate(api_key: str, path: str):
    # allowlist short-circuit (MVP)
    allowed = {k.strip() for k in (os.getenv("R4_ALLOWED_KEYS") or "").split(",") if k.strip()}
    if api_key in allowed:
        return 200, {"ok": True, "plan": "allowlist"}
...
class BillingGuardMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        if not api_key:
            # DEMO: allow-no-key for demo surfaces -> bypass billing guard (rate-limit middleware still applies)
            if (os.getenv("DEMO_ENABLED") or "0") == "1":
                allow_no_key = (os.getenv("DEMO_ALLOW_NO_KEY") or "0") == "1"
                if allow_no_key and _is_demo_surface(request):
                    return await call_next(request)
            return JSONResponse({"detail":"missing_api_key"}, status_code=401)

        # Billing is only for paid keys (r4_*). Non-r4 keys are handled by allowlist in require_api_key().
        if not api_key.startswith("r4_"):
            return await call_next(request)
...
async def require_api_key(
    ...
    """
    Auth policy (MVP):
      - demo surfaces: if DEMO_ALLOW_NO_KEY=1, missing key is treated as DEMO_KEY
      - otherwise: key required
      - allowlist (R4_ALLOWED_KEYS) is DEV-ONLY: enforced only when env is non-empty
      - paid/billing decisions are enforced by BillingGuardMiddleware (billing gate)
    """
    ...
    # normalize path for /api/v1/*
    path = request.url.path or ""
    if path.startswith("/api/v1/"):
        norm = "/v1/" + path[len("/api/v1/"):]
    else:
        norm = path

    sig = (request.query_params.get("sig") or "").strip().lower()
    is_demo_surface = (norm == "/v1/random") or (norm == "/v1/vrf" and sig == "ecdsa")

    # DEMO: explicit demo key OR allow-no-key
    if demo_enabled and is_demo_surface:
        if api_key == demo_key and demo_key:
            return demo_key
        if (not api_key) and demo_allow_no_key and demo_key:
            return demo_key

    if not api_key:
        raise HTTPException(status_code=401, detail="missing_api_key")

    # DEV allowlist only when set
    raw = (os.getenv("R4_ALLOWED_KEYS") or "").strip()
    if raw and not api_key.startswith("r4_"):
        allowed = {k.strip() for k in raw.split(",") if k.strip()}

        demo_api = (os.getenv("DEMO_API_KEY") or "").strip()
        if demo_api:
            allowed.add(demo_api)

        pub = (os.getenv("PUBLIC_API_KEY") or "").strip()
        if pub:
            allowed.add(pub)
    ...
    # allowlist is demo/public-only; paid keys are enforced by BillingGuardMiddleware (/gate/allow)
